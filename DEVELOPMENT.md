...
mkdir ~/dev/kibana-6.8.8-release
cd ~/dev/kibana-6.8.8-release/
gunzip -c kibana-6.8.8-darwin-x86_64.tar.gz | tar xopf -
cd kibana-6.8.8-darwin-x86_64

# Install the built plugin
./bin/kibana-plugin install 'file:///<your-path>/kibana-6.x-git/kibana-extra/kibana-milestones-vis/build/kibana-milestones-vis-6.8.8.zip'
...
git commit -m "Bump version to 6.8.8."
git tag v6.8.8
git push origin 6.8
git push --tags

# On Github, edit the new release at
# https://github.com/walterra/kibana-milestones-vis/releases/new?tag=v6.8.8
# Use `Kibana v6.8.8 compatibility release.` as the release text.
# Add the build file `kibana-milestones-vis-6.8.8.zip` to the releases' binaries.
